...
## ⚠️ Важно: Этот код сгенерирован ИИ

> **Весь код в этом репозитории был сгенерирован искусственным интеллектом как РЕФЕРЕНС (пример).**
> Он нужен для того, чтобы каждый участник команды мог:
>
> 1. **Посмотреть**, как должен выглядеть итоговый проект
> 2. **Понять** структуру файлов, классов и логику взаимодействия
> 3. **Увидеть**, какой результат мы хотим получить
>
> **Мы будем писать свой код с нуля, своими руками.** Этот код — только ориентир.

### Как использовать этот референс

- Запустите проект (инструкция ниже) и покликайте — посмотрите как оно работает
- Изучите структуру папок и файлов — мы будем делать так же
- Посмотрите логику контроллеров и моделей — пишите свой код по аналогии
- **НЕ копируйте код напрямую** — пишите сами, понимая что и зачем вы делаете

---

## 🖥 Как запустить этот референс-код

### 📌 Нужно установить

1. **Qt 6** — скачать с [qt.io](https://www.qt.io/download-open-source)
...
```bash
qmake StudyTable.pro   # генерируем Makefile
make                   # собираем
open StudyTable.app    # запускаем
```

### 🪟 На Windows

```cmd
:: Откройте "Qt 6.x.x (MinGW)" из меню Пуск
...
Или через **Qt Creator** (проще всего, работает на обеих ОС):

1. Откройте Qt Creator → `File` → `Open File or Project` → `StudyTable.pro`
2. Выберите комплект (Kit) → нажмите ▶ (Run)
